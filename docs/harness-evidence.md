...
| Continuation persistence | Capabilities `session::tests::continuation_preference_survives_session_resume`；Gateway `test_session_catalog_reads_bounded_history_without_web_state` | `thread_settings.json` 按 Thread ID 原子保存；SessionStore resume 与 Gateway read-only projection 均返回同一 bounded mode | covered for sidecar/resume/projection; full App Server process shutdown/restart remains a lifecycle follow-up |
