...
| Cross-client revision projection | App Server `stateRevision` 经 Python SDK、Gateway route 和 Web Studio settings/Goal notification 到达所有客户端；各客户端按 Thread 单调消费，重连时重建 cursor | stale SDK notification 或 Web notification 覆盖较新的 Plan/continuation/Goal state | covered for settings/Goal and Web reconnect rebuild; Gateway-internal restart generation signal remains |
