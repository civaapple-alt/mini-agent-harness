...
| Continuation persistence | Capabilities `session::tests::continuation_preference_survives_session_resume`；App Server `json_rpc::tests::persists_and_restores_thread_continuation_through_app_server_restart`；Gateway `test_session_catalog_reads_bounded_history_without_web_state` | `thread_settings.json` 按 Thread ID 原子保存；显式 idle shutdown 释放旧 worker lock；新 App Server bind、SessionStore resume 与 Gateway read-only projection 均返回同一 bounded mode | covered; active-turn shutdown intentionally returns `Busy` |
