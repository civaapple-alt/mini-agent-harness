...
| Continuation persistence | Capabilities `session::tests::continuation_preference_survives_session_resume`；App Server `json_rpc::tests::persists_and_restores_thread_continuation_through_app_server_restart`、`json_rpc::tests::broadcasts_thread_settings_updates_with_action_revision`、`json_rpc::tests::enforces_goal_timeout_with_cooperative_cancellation`；Gateway `test_session_catalog_reads_bounded_history_without_web_state` | `thread_settings.json` 按 Thread ID 原子保存；显式 idle shutdown 释放旧 worker lock；active turn 返回 `Busy`，settlement 后可 shutdown；两个 App Server subscriber 收到同一 revision；新 bind、SessionStore resume 与 Gateway read-only projection 均返回同一 bounded mode | covered; SDK/Gateway/Web 跨进程 revision 收敛与跨 Project/Thread attach remain |
