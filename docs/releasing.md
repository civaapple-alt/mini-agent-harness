...
- [ ] `CHANGELOG.md` has a dated `0.6.0` section and an empty `Unreleased`
...
rg -n '^## \[(Unreleased|0\.6\.0)\]' CHANGELOG.md
```
...
[v0.6.0 release page](https://github.com/civaapple-alt/mini-agent-harness/releases)
...
```sh
shasum -a 256 -c mini-agent-v0.6.0-<target>.tar.gz.sha256
tar -xzf mini-agent-v0.6.0-<target>.tar.gz
./mini-agent-v0.6.0-<target>/mini-agent --version
```

On Windows PowerShell:

```powershell
Get-FileHash .\\mini-agent-v0.6.0-x86_64-pc-windows-msvc.zip -Algorithm SHA256
Expand-Archive .\\mini-agent-v0.6.0-x86_64-pc-windows-msvc.zip .\\mini-agent-v0.6.0
.\\mini-agent-v0.6.0\\mini-agent.exe --version
```

Confirm that `--version` reports `0.6.0`. Then announce the release with a
