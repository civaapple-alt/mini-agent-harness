...
REPL uses its local approval adapter. Noninteractive `run` fails closed on
sensitive tools unless `run --auto-approve` (or `-y`). Managed-process tools
are not part of the Builtin catalog. Core keeps an internal bounded loop guard;
there is no user-configurable step budget or unattended CLI mode.
...
review extension code and endpoints before approval and use automatic approval
only with trusted extensions.
