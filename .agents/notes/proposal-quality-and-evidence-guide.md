...
[ ] 已写至少一个会失败的验收标准和一个反例
[ ] 已检查是否可以删除旧概念或兼容路径
[ ] canonical 模型优先于兼容实现，内外词汇一一对应且旧输入拒绝行为已写
...
当旧字段、旧枚举或旧缓存无法表达新的正交语义时，提案应先建立完整的 canonical
模型，再一次性对齐内部实现与外部契约。避免为了“兼容”保留旧字段、增加别名、双写、
隐式映射或静默 fallback；这类胶水层会让所有权、失效边界和安全结论分裂，未知输入
还可能被默认值放行。

提案至少要写清：

- canonical 状态、所有者、写入者、消费者，以及明确禁止的重复实现；
- 内部类型、协议、SDK、网关和 UI 的一一映射，包含版本边界；
- 旧输入、缺失字段和未知值的明确拒绝行为（权限相关默认 fail closed）；
- 若确实需要临时适配层，其删除条件、有效期、风险和对应的失败反例。

如果方案只能依赖“保留旧概念，先做一层转换再逐步迁移”，状态应保持为草稿/提案中，
不得把局部可运行写成已实现；必须补齐内外一致性和跨层证据后才能晋级。
