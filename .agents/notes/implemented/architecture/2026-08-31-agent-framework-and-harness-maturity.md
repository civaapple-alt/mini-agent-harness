...
| all Rust source | 28,270 / 30,000 | 94.2% | 1,730 |

组成明细：

- runtime：production 11,719，unit 3,517，integration 0；合计 15,236。
- all Rust source：production 21,242，unit 5,887，integration 1,141；合计 28,270。
...
2. **阶段 1：先释放全局预算——进行中**。目标是净减少约 2,000 行，将全 Rust 总量降至约 26,900 行。相对本次整理前的 28,934 行，已释放 664 行，距离目标还差约 1,370 行；当前仍不能恢复常规扩展节奏。
...
本批验证：`cargo test -p mini-agent-capabilities`（61 passed）、`cargo test -p mini-agent-host`（40 passed）、两包 Clippy、`cargo fmt --all`、`python scripts/line_budget.py` 均通过。累计阶段 1 释放量按门禁脚本从 642 行更新为 664 行；本批不删除 Core 核心测试，也不移动 Actor/CAS/Session 边界。
