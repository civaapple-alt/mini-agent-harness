...
| 范围 | 当前 / 上限 | 使用率 | 剩余 |
| :--- | ---: | ---: | ---: |
| runtime（`core + protocol + host + app-server`） | 15,243 / 20,000 | 76.2% | 4,757 |
| all Rust source | 28,292 / 30,000 | 94.3% | 1,708 |

组成明细：

- runtime：production 11,719，unit 3,524，integration 0；合计 15,243。
- all Rust source：production 21,242，unit 5,909，integration 1,141；合计 28,292。
...
2. **阶段 1：先释放全局预算——进行中**。目标是净减少约 2,000 行，将全 Rust 总量降至约 26,900 行。相对本次整理前的 28,934 行，已释放 642 行，距离目标还差约 1,392 行；当前仍不能恢复常规扩展节奏。
...
| App Server default-thread facade | `thread_read/update/close`、`turn_start/steer/cancel` 仅把默认 `self.thread_id` 转发到显式 `*_for` API | 删除一次性默认线程 wrapper，测试改走显式线程 ID；净释放 17 行 | 不改变 App Server actor、CAS/revision、Session 或 JSON-RPC 协议 |
| `SandboxKind` / `SecurityPreset` 字符串入口 | `as_str` 是 `name` 的重复公共别名 | 删除 accessor，Host 统一使用 canonical `name()`，并删除两条静态名称断言；净释放 10 行 | 配置 `parse` 别名未改，公共 JSON-RPC 不变 |
...
| App Server duplicate test fixtures | `workflow_connection` 和 `management_connection` 仅以不同标签重复创建同一 managed connection | 合并为参数化 fixture；净释放 8 行 | 保留全部 23 个 App Server/JSON-RPC 行为测试 |
| App Server WorldState projection | `world_state_value` 和 `world_state_result_action` 重复拼装相同 `WorldStateResult` | 复用私有投影函数；按门禁净释放 1 行 | 不改变 action metadata、世界状态字段或错误路径 |
