...
| 范围 | 当前 / 上限 | 使用率 | 剩余 |
| :--- | ---: | ---: | ---: |
| runtime（`core + protocol + host + app-server`） | 15,412 / 20,000 | 77.1% | 4,588 |
| all Rust source | 28,517 / 30,000 | 95.1% | 1,483 |

组成明细：

- runtime：production 11,896，unit 3,516，integration 0；合计 15,412。
- all Rust source：production 21,445，unit 5,931，integration 1,141；合计 28,517。

全 workspace 的 30,000 行门禁已经是当前实际瓶颈：可用余量只有 1,483 行，约为总预算的 4.9%。因此后续新增能力不能只看 runtime 还剩 4,588 行，还必须同时计入 CLI、测试和 integration 的全局增长。
...
2. **阶段 1：先释放全局预算——进行中**。相对本次整理前的 28,934 行，已释放 417 行；当前仍需继续形成足够的维护余量，不能恢复常规扩展节奏。
...
1. **行数预算**：runtime 上限 20,000 行，workspace 上限 30,000 行；当前分别使用 77.1% 和 95.1%，这是复杂度门禁，不是扩展目标。
