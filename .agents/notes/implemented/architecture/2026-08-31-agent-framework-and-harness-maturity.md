...
2. **阶段 1：先释放全局预算——进行中**。目标是净减少约 2,000 行，将全 Rust 总量降至约 26,900 行。相对本次整理前的 28,934 行，已释放 600 行，距离目标还差约 1,434 行；当前仍不能恢复常规扩展节奏。
...
本批验证：`cargo test -p mini-agent-host`（41 passed）、`cargo test -p mini-agent-app-server`（23 passed）、两包 Clippy、`cargo fmt --all`、`python scripts/line_budget.py` 均通过。累计阶段 1 释放量按门禁脚本从 585 行更新为 600 行；本批不删除 Core 核心测试，也不移动 Actor/CAS/Session 边界。
