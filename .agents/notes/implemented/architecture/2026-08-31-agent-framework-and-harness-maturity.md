...
| all Rust source | 28,645 / 30,000 | 95.5% | 1,355 |

组成明细：

- runtime：production 11,895，unit 3,598，integration 0；合计 15,493。
- all Rust source：production 21,444，unit 6,060，integration 1,141；合计 28,645。

全 workspace 的 30,000 行门禁已经是当前实际瓶颈：可用余量只有 1,355 行，约为总预算的 4.5%。因此后续新增能力不能只看 runtime 还剩 4,507 行，还必须同时计入 CLI、测试和 integration 的全局增长。
...
2. **阶段 1：先释放全局预算——进行中**。相对本次整理前的 28,934 行，已释放 289 行；当前仍需继续形成足够的维护余量，不能恢复常规扩展节奏。
...
3. 测试不是免费的预算：unit/integration 测试分别占 all Rust source 的 21.2% 和 4.0%，新增测试需要和生产代码一起纳入门禁评估。
...
1. **行数预算**：runtime 上限 20,000 行，workspace 上限 30,000 行；当前分别使用 77.5% 和 95.5%，这是复杂度门禁，不是扩展目标。
