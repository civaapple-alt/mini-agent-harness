...
Related: [context compaction](../feature/2026-08-24-context-compaction.md), [durable sessions](../feature/2026-08-24-durable-sessions-and-recovery.md), [provider seams](../feature/2026-08-27-deepseek-and-glm-provider-seams.md), [hard limits](2026-08-24-hard-limits-system.md).
...
| `read_image` | Envelope only. Pixels are not in core. | `ImageStore` plus `session/attachments/{id}{ext}` and `{id}.json`. Wire: DeepSeek `file_id`, GLM data URL. |
...
The compact auxiliary request sends `tools: []`. Host therefore does not project images and cannot call `web_fetch` / `read_image`. GLM compact stays on Responses. Mentor was already empty-catalog.

## Consequences

- GLM `read_image` survives process restart and `fork` without copying the original file into the workspace.
- Compact cannot attach data URLs or switch to Chat Completions.
- The 1 MiB JSON ceiling still does not include GLM data URLs on normal image turns; that remains a host wire payload.
- Prefix cache: catalog is stable; GLM image turns still miss cache versus the previous Responses text turn (model + endpoint change). Compaction still replaces the message prefix.
