...
1. **Lineage-Preserving Forking**: In [`crates/mini-agent-cli/src/session.rs`](crates/mini-agent-cli/src/session.rs), `SessionStore::open(..., SessionRequest::Fork(parent_id))` reads the parent session's latest settled checkpoint and initializes a new discrete session directory and file.
...
- Verified by automated unit tests in [`crates/mini-agent-cli/src/session_tests.rs`](crates/mini-agent-cli/src/session_tests.rs) and CLI dispatch tests in [`crates/mini-agent-cli/src/main.rs`](crates/mini-agent-cli/src/main.rs).
