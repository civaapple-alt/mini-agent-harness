...
- all Rust source: `29,360 / 30,000` lines (97.9%; 640 remaining)
...
The first bounded scenario baseline is now implemented and recorded in the [harness iteration note](implemented/architecture/2026-08-31-vscode-harness-lessons-next-iteration.md): 8 representative CLI scenarios pass, with the original App Server baseline 28/28 and current App Server 29/29 plus CLI interactive 13/13 regression evidence. CLI automatic Trace report wiring, HTTP 429 retry/backoff policy, actual MCP call denial, Docker sandbox availability/isolation, and model/provider comparison remain explicitly open gaps; the Core/Capabilities fault paths (including bounded HTTP 429 classification, MCP connection refusal, and pre-sandbox shell refusal), CLI unknown-tool recovery, bounded cross-file refactor, and App Server `NeedsApproval` path are covered separately.
