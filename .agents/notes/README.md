...
- runtime (`core + protocol + host + app-server`): `16,556 / 20,000` lines (82.8%; 3,444 remaining)
- all Rust source: `29,258 / 30,000` lines (97.5%; 742 remaining)
...
3. Preserve the first `ToolExecutionDelegate`/Host `ToolOrchestrator` slice and the
   Shell typed-admission migration. Before moving another tool, add the real App
   Server public Shell approval scenario and define `requestId`/`turnId`/`callId`
   correlation plus non-blocking approval behavior. Do not add a generic
   router/sandbox wrapper or leave two approval authorities active.
