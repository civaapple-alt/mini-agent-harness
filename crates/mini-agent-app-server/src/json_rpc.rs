    ...
    M: Model + Send + 'static,
{
    ...
    async fn handle_request_inner(&mut self, request: JsonRpcRequest) -> Option<JsonRpcResponse> {
        ...
        match request.method.as_str() {
            ...
            METHOD_SESSION_FORK => self.handle_session_fork(request).await,
            METHOD_SESSION_INFO => self.handle_session_info(request).await,
            METHOD_SESSION_NOTEBOOK_READ => self.handle_session_notebook_read(request).await,
            METHOD_SESSION_NOTEBOOK_WRITE => self.handle_session_notebook_write(request).await,
            METHOD_SESSION_NOTEBOOK_FORGET => self.handle_session_notebook_forget(request).await,
