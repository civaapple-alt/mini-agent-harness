...
#[tokio::test]
async fn zero_max_steps_halts_immediately_at_step_limit() {
    let model = ScriptedModel {
        responses: VecDeque::from([ModelResponse {
            reasoning: String::new(),
    ...
    let outcome = harness.run("halt", &mut ()).await.unwrap();
...
#[tokio::test]
async fn unbounded_max_steps_runs_until_the_model_completes() {
    let model = ScriptedModel {
        responses: VecDeque::from([
            ModelResponse {
                reasoning: String::new(),
            ...
            ModelResponse {
                reasoning: String::new(),
                text: "done after two steps".to_string(),
                tool_calls: Vec::new(),
                usage: None,
            },
        ]),
    ...
    assert_eq!(outcome.stop_reason, StopReason::Completed);
    assert_eq!(outcome.steps, 2);
    assert_eq!(outcome.final_text, "done after two steps");
}
